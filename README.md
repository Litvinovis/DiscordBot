...
| `+пополнить <сумма>` | Пополнить счёт (до 200 000 ₽, раз в 30 дней; в доходность не засчитывается) |
| `+купить-валюту <КОД> <сумма ₽>` / `+продать-валюту <КОД> <кол-во>` | Обмен валюты по курсу ЦБ |
| `+валюта-портфель` | Валютные остатки |
...
Счёт ведётся в рублях. Иностранные бумаги (котировки в $, € и т.д.) покупаются и продаются
по курсу ЦБ РФ; средняя цена позиции и P&L по бумаге показываются в её валюте, итоги — в рублях.
