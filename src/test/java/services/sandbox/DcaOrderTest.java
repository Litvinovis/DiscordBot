...
public class DcaOrderTest {
    ...
    void testWeeklyOrder_nextExecutionIn7Days() {
        ...
        DcaOrder order = new DcaOrder("user1", "SBER", 5000.0, "WEEKLY", nextExecution, now, true);
    ...
    void testMonthlyOrder_nextExecutionIn30Days() {
        ...
        DcaOrder order = new DcaOrder("user1", "LKOH", 10000.0, "MONTHLY", nextExecution, now, true);
    ...
    void testFindDueOrders_returnsOnlyOverdueOrders() {
        ...
        DcaOrder overdue = new DcaOrder("user1", "SBER", 5000.0, "WEEKLY",
                now.minus(1, ChronoUnit.HOURS), now.minus(8, ChronoUnit.DAYS), true);
        DcaOrder notDue = new DcaOrder("user2", "GAZP", 3000.0, "WEEKLY",
                now.plus(6, ChronoUnit.DAYS), now, true);
        DcaOrder inactiveDue = new DcaOrder("user3", "LKOH", 7000.0, "MONTHLY",
    ...
    void testDcaOrder_fieldsStoredCorrectly() {
        ...
        DcaOrder order = new DcaOrder("u42", "NVDA", 2500.0, "WEEKLY", next, now, true);
        order.setId(99L);

        assertEquals(99L, order.getId());
        assertEquals("u42", order.getUserId());
        assertEquals("NVDA", order.getTicker());
        assertEquals(2500.0, order.getAmountRub(), 0.001);
    ...
    void testDcaOrder_cancel_setsActiveToFalse() {
        Instant now = Instant.now();
        DcaOrder order = new DcaOrder("u1", "SBER", 5000.0, "WEEKLY",
